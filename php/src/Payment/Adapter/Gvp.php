...
class Gvp extends AdapterAbstract implements AdapterInterface
{
    ...
    private function _buildBaseRequest(Request $request)
    {
        ...
        return array ('Version' => '0.01','Mode' => $this->_config->mode,
    ...
    private function _buildTerminal(Request $request)
    {
        ...
        return array ('ProvUserID' => $username,'HashData' => $hash,
                'UserID' => $username,'ID' => $config->terminal_id,
    ...
    {
        return array ('OrderID' => $request->getOrderId(),'GroupID' => null,
                'Description' => null 
        );
    }
     ...
     * @return array
     */
    private function _buildTransaction(Request $request, 
            $cardHolderPresentCode = 0, $originalRetrefNum = null)
    {
        $transactionType = $request->getTransactionType();
        $installment = ($request->getInstallment()) ? $this->_formatInstallment(
            $request->getInstallment()) : null;
        $amount = $this->_isAmountRequired($request) ? $this->_formatAmount(
            $request->getAmount()) : 0;
        $currency = ($amount) ? $this->_formatCurrency($request->getCurrency()) : null;
        $type = $this->_getProviderTransactionType($transactionType);
        
        return array ('Type' => $type,'InstallmentCnt' => $installment,
                'Amount' => $amount,'CurrencyCode' => $currency,
                'CardholderPresentCode' => $cardHolderPresentCode,
                'MotoInd' => 'N','OriginalRetrefNum' => $originalRetrefNum 
        );
    }
    ...
    {
        $isAuth = in_array($transactionType, 
            array (self::TRANSACTION_TYPE_SALE,
                    self::TRANSACTION_TYPE_PREAUTHORIZATION,
                    self::TRANSACTION_TYPE_POSTAUTHORIZATION 
            ));
    ...
    private function _getTransactionHash(Request $request, $password)
    {
        ...
        return strtoupper(
            sha1(
                sprintf('%s%s%s%s%s', $orderId, $terminalId, $cardNumber, 
                    $amount, $securityData)));
    }
    ...
    {
        $rawRequest = call_user_func(
            array ($this,$requestBuilder 
            ), $request);
    ...
    protected function _parseResponse($rawResponse)
    {
        ...
        try {
            ...
            $exception = new UnexpectedResponse(
                'Provider is returned unexpected ' . 'response. Response data:' .
                         $rawResponse);
        ...
        $response->setIsSuccess(
            (string) $xml->Transaction->Response->Code == '00');
    ...
    {
        return (! $reverse) ? number_format($amount, 2, '', '') : (float) substr(
            $amount, 0, - 2) . '.' . substr($amount, - 2);
    }
    ...
    {
        return sprintf('%02s%02s', $month, substr($year, - 2));
    }
